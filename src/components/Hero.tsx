...
export default function Hero() {
  ...
  const glanceItems = [
    ...
    { label: t('glanceTransactions'), value: '26K+', icon: 'M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z' },
        ...
        <div className="grid lg:grid-cols-12 gap-10 items-center">
          <div className="lg:col-span-7">
            ...
            <h1 className="text-4xl md:text-6xl leading-tight text-[var(--ink-900)] mb-6">
              {t('titleLine1')}
              <br />
              {t('titleLine2')}<span className="text-gradient">{t('titleHighlight')}</span>
            </h1>
            ...
            <div className="relative rounded-3xl border border-[var(--line)] bg-[var(--surface)] p-6 md:p-8 shadow-[0_36px_70px_-42px_rgba(31,27,22,0.85)]">
              ...
              <div className="space-y-3">
                {glanceItems.map((item) => (
                  <div key={item.label} className="flex items-center justify-between rounded-xl bg-white p-4 border border-[var(--line)]">
                    <div className="flex items-center gap-3">
                      <svg className="w-5 h-5 text-[var(--accent-500)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={item.icon} />
                      </svg>
                      <span className="text-[var(--ink-700)]">{item.label}</span>
                    </div>
                    <strong className="text-xl text-[var(--ink-900)]">{item.value}</strong>
                  </div>
                ))}
              </div>
            </div>
